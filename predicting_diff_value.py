...
def main(predicting="v(Binance Coin)", test_size=0.01):
    ...
    predicting_label = f"predicting_diff_{predicting}"
    df[f"diff({predicting})"] = df[predicting].diff()
    prediction_df = pd.DataFrame(
        {"Date": df.index + pd.DateOffset(1), predicting_label: df[f"diff({predicting})"]}
    )
    ...
    df.drop(predicting_label, axis=1, inplace=True)
    ...
    plt.title(f"Predicting diff {predicting} ({y.shape[0] - divider} days from past {divider} days of data)")
    ...
    plt.savefig(f"./plots/predicting_diff_{predicting}.png")
